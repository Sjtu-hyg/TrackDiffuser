...
class GaussianDiffusion(nn.Module):
    def __init__(self, model, horizon, observation_dim, action_dim, n_timesteps=1000,
        loss_type='state_l24', clip_denoised=False, predict_epsilon=False,
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, cond, returns, test_input, verbose=True, return_diffusion=False): #test_input是1：t的观测，cond只有1：t-1的观测，这里test_input只是用来计算mask，覆盖值用的
        device = returns.device

        batch_size = shape[0]
        x = 0.5*torch.randn(shape, device=device) #old
        # x = 1 * torch.randn(shape, device=device) #new
        # x = x + test_input #new1
        mask = test_input != 0
        x = x * mask.float()
        ...
        for i in reversed(range(0, 500)):
            timesteps = torch.full((batch_size,), 2*i, device=device, dtype=torch.long)
    ...
    def p_losses(self, x_start, cond, t, returns=None):
        ...
        x_recon = x_recon * mask.float()

        if not self.predict_epsilon:
            x_recon = apply_conditioning(x_recon, cond, self.action_dim)
            # x_recon = x_recon #loss22
...
class GaussianInvDynDiffusion(nn.Module):
    ...
    def loss(self, x, cond, returns=None):
        if self.train_only_inv:
            ...
            batch_size = len(x)
            t = torch.randint(0, self.n_timesteps, (batch_size,), device=x.device).long()
            diffuse_loss, info = self.p_losses(x[:, :, :], cond, t, returns)  #list = [1, 2, 3, 4, 5] print(list[1:4])  # Output: [2, 3, 4]
            # Calculating inv loss
            x_t = x[:, :-1, self.action_dim:]
            a_t = x[:, :-1, :self.action_dim]
            x_t_1 = x[:, 1:, self.action_dim:]
