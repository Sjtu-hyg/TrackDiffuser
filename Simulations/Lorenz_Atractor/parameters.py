...
def f(x, jacobian=False):
    ...
    if jacobian:
        ...
        return torch.bmm(F, x)
def f_batch(x, jacobian=False):


    BX = torch.zeros([x.shape[0],x.shape[1],m,m]).float().to(x.device) #[batch_size,horizon, m, m]
    BX[:,:,1,0] = torch.squeeze(-x[:,:,2,:])
    BX[:,:,2,0] = torch.squeeze(x[:,:,1,:])
    Const = C.to(x.device)
    Const_all = Const.reshape((1, 1, m, m)).repeat(x.shape[0], x.shape[1], 1, 1)
    A = torch.add(BX, Const_all)
    # Taylor Expansion for F
    F = torch.eye(m).to(x.device)
    F = F.reshape((1, 1, m, m)).repeat(x.shape[0], x.shape[1], 1, 1) # [batch_size, m, m] identity matrix
    for j in range(1,J+1):
        F_add = (torch.matrix_power(A*delta_t, j)/math.factorial(j))
        F = torch.add(F, F_add)
    if jacobian:
...
def h_nonlinear(x, jacobian=True):
